...
public class PurchaseService {
    ...
    private Double computeAmount(List<Pizza> pizzas) {
        ...
        totalPrice = countDiscount(pizzas,totalPrice,3);
        return totalPrice;
    }

    private Double countDiscount(List<Pizza> pizzas, Double totalPrice, int discountNumberBase) {
        if (pizzas.size() < discountNumberBase)
            return totalPrice;
        List<Pizza> sortedPizza = pizzas.stream()
                .sorted(Comparator.comparing(Pizza::getPrice))
                .collect(Collectors.toList());
        int numberOfFree = pizzas.size() / discountNumberBase;
        for (int i = 0; i < numberOfFree; i++) {
            totalPrice -= sortedPizza.get(i).getPrice();
        }
        return totalPrice;
    }
